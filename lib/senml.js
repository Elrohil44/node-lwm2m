...
function serializeToString(obj, schema) {
  ...
  function append(arr, key, value, type) {
    var types = {
      String: function() {
  ...
  for (var i = 0; i < keys.length; ++i) {
    ...
    append(result.e, String(res.id), obj[key], res.type);
  }

  if (schema.basename) {
    result.bn = schema.basename;
  }
...
function parse(payload, schema) {
  ...
  if (obj.bn) {
    if (obj.bn === '/') {
      ...
      offset = 0;
    } else if (utils.isResource(obj.bn)) {
      id = obj.bn.split('/').slice(-2)[0];
      type = schema.resources[id].type;
      if (!Array.isArray(type)) {
        throw new Error('Invalid JSON payload');
      }
      result = [];
  ...
  for (var i = 0; i < obj.e.length; ++i) {
    var key = offset === -1 ? id : schema.id[obj.e[i].n.split('/')[offset]];
    ...
    if (offset === -1) {
      append(result, obj.e[i].n, { type: type[0] }, obj.e, i);
    } else {
